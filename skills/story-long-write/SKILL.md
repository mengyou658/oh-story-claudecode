...
| 场景 | 触发条件 | 执行流程 |
|------|----------|----------|
| **开书** | "帮我开书" / 项目目录为空 | Phase 1→2→3：建项目、核心设定（含**标题脑洞×3 + 每条附脑洞描述 + 用户选 3 卖点杂糅**）、卷纲与首批细纲；**默认停在细纲交付，不自动写正文** |
...
**大纲生成要求：其中包含三次大两难性时刻以及80个小的冲突(默认80个主清单+20个备选；用户可以自己描述至少多少个冲突)(可以是自我或者外部爆发升级反转或者处理)以及一个清晰的道德前提。**  
**细纲生成要求：每章节都有一个小冲突(可以是自我或者外部爆发升级（反转或处理或打脸或突然想起重要的事情忘记处理或者其他的冲突）)。**

**执行前先读 [references/workflow-setup.md](references/workflow-setup.md) 的「Phase 3：大纲搭建」节**（含上述硬约束与字段模板）；本仓库另见 [references/local-production-rules.md](references/local-production-rules.md) §0（冲突配额）与 §0b（地名虚构）。
