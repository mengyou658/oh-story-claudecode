...
生成书名候选时产量 **×3**：默认惯例 3 个 → 实际出 **9** 个；用户要 N 个 → 出 **3N** 个。用户从中选 **3** 个，分别定为前期卖点 / 后期卖点 / 贯穿卖点，杂糅进同一本书（见 `story-long-write/references/local-production-rules.md` 与 `workflow-setup.md`「标题脑洞」）。
