...
1. **读取写作计划**：写作计划按 **卷契约 → 当前剧情单元 → 章节细纲** 的顺序确定；已写内容仍以正文与 `追踪/` 文件为准。先读 `追踪/章号映射.md` 确认正文进度 B 与细纲消费进度 M。本批「写 K 章」指 **K 章正文**（默认消费 `3K` 章细纲）。先按 SKILL.md「卷纲不整读」跑 `outline_view.py --unit {单元ID} --stage write {卷纲路径}` 取卷契约、全卷常任裁定、当前剧情单元（单元ID/位置）、单元情绪引擎、本卷主推线/战果与终局底牌边界（单元ID 从本批首组细纲「单元ID/位置」取），再按正文章号逐章加载对应三份细纲（正文 B → 细纲 3B-2…3B）。卷纲或细纲缺少当前协议的必需字段时按既有流程先补齐，未知字段写 `[待补充]`；已锁定卷纲不得自动改动。细纲读取阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化的语义义务/执行边界及结尾钩子，不读取或心算逐点字数预算。只有真正影响后续连续性的结果才进入本章事务。硬规则见 [local-production-rules.md](local-production-rules.md)。
...
2. **逐章执行**：每章按 [workflow-chapter.md](workflow-chapter.md) 的「单章写作流程」步骤 1-13 完整走一遍（写前准备、正文执行、字数验证、钩子/爽点检查、元信息与禁用词扫描、追踪事务）。批量模式下再叠加下列日更专属动作：
   ...
   - 每章写完后**立即提交一次追踪事务**：
     ...
     4. 运行 `storyctl.py chapter check`，取 `state_revision` 写入无 `wordcount` 的事务；并用 `length.actual` 写入 `追踪/字数记录/第{NNN}章.md`（只记账）。`under`/`over` 相对细纲目标 → 默认 `accept-current-length`（不追目标、不加厚、不因 <2000 合并）；`over` 仅在明显注水时按 workflow-chapter 步骤 8 可选净删一次。提交会重读、重数、重跑 quality；成功后删临时 JSON，**更新 `追踪/章号映射.md`**，tracking 提交后才继续。
   ...
   - **质检提示**：本章写作完成 = 同轮 deslop 定稿覆盖已完成。如需一致性检查，运行 `/story-review lean`。批量写作模式跳过 review，但每章仍须同轮完成 deslop 定稿，不可攒到批末再去味。
3. **不中断但不并发**：tracking 已提交即进下一章（除非用户要求确认）；一次压缩后仍明显注水则展示证据/动作，不静默推进。相对目标 `under` 不阻断（已记账并 accept）。下一章先读上一章正文与追踪更新。
