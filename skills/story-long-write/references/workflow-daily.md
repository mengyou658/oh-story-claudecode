...
2. **逐章执行**：每章按 [workflow-chapter.md](workflow-chapter.md) 的「单章写作流程」步骤 1-13 完整走一遍（写前准备、正文执行、字数验证、钩子/爽点检查、元信息与禁用词扫描、追踪事务）。批量模式下再叠加下列日更专属动作：
   ...
   - 每章写完后**立即提交一次追踪事务**：
     ...
     4. 运行 `storyctl.py chapter check`，取 `state_revision` 写入无 `wordcount` 的事务。`under` 且实际 ≥2000 → `accept-current-length`（不追目标、不水话）；`under` 且 <2000 → 禁止加厚；`over` 按 workflow-chapter 步骤 8 净删一次并复检（优先紧凑）。提交会重读、重数、重跑 quality；成功后删临时 JSON，**更新 `追踪/章号映射.md`**，tracking 提交后才继续。
