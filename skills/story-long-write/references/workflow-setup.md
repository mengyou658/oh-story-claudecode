...
| **20个小的冲突** | 编号列表；每个注明类型（自我 / 外部）与形态（爆发 / 升级 / 反转 / 处理）；可锚定到卷或剧情单元，未排细纲的可写计划落点 |

三次大两难应落在不同阶段（建议开篇末/发展中段/高潮前各一），不得挤在同一剧情单元。20个小冲突覆盖全书规划弧，首批细纲至少兑现其中对应章区间的条目。
...
- **大纲/大纲.md**：全书卷级鸟瞰（卷名+字数+章数+核心事件+状态变化，一段式汇总）+ **道德前提 + 三次大两难性时刻 + 20个小冲突清单**（大纲生成要求）
