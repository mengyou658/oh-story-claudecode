...
2. 请用户**选出 3 个**（可改写）。
3. 与用户确认三者职责并落盘到核心设定表「书名卖点分配」：**1 个前期、1 个后期、1 个整本书贯穿**——三个卖点杂糅进**同一本**故事，不得写成三本书或三选一丢弃。
4. 再从三选中定 **1 个对外书名**（目录名）；另两个仍作为必须兑现的卖点义务写入 `设定/题材定位.md`。

用户已书面给定唯一书名时：可跳过扩量生成，但仍须拆出/补全「前期 / 后期 / 贯穿」三卖点登记。
...
- 书名卖点分配：
  - 前期卖点：{用户三选之一；黄金三章主兑现}
  - 后期卖点：{用户三选之一；中后段/终局兑现}
  - 贯穿卖点：{用户三选之一；全书主线持续兑现}
- 题材/类型：{主类型 + 副类型}
- 目标平台：{起点/番茄/晋江/其他}
- 预计字数：{X} 万字
- 目标读者：{画像}
