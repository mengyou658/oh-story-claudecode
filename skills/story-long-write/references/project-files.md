...
│   ├── 章号映射.md                ← 正文进度 ↔ 细纲消费进度（本仓库）
│   ├── 字数记录/第NNN章.md          ← 每章 visible_chars_v1 独立账本（不追写）
...
| 追踪/章号映射.md | 全书 | Phase 4 首次正文起维护 | **正文进度 + 细纲消费进度**双记录；默认 3 细纲→1 正文；每次 commit 后更新（见 local-production-rules.md） |
| 追踪/字数记录/第NNN章.md | 章 | Phase 4 每章定稿 | 只记实际字数与路径；不追目标、不触发加厚；与 tracking 内 wordcount 并行 |
...
- 每章写完并完成同轮 deslop 定稿覆盖后，正式正文已在 `正文/`；不要只把未去味稿留给对话或 `_humanized` 旁路文件当作交付
