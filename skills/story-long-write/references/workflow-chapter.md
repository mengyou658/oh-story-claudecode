...
> **本仓库默认：正文第 B 章 = 合并细纲第 `(3B-2)`–`(3B)` 章**（权威：[local-production-rules.md](local-production-rules.md) §2）。下文「第 N 章」若指用户说的正文章号，则细纲取三份；用户书面要求一纲一章时才读单份。

0. **解析章号映射**：正文章号 `B`；细纲区间 `A=3B-2 … C=3B`。确认三份细纲均存在；缺则先补纲。读/建 `追踪/章号映射.md`。
1. **检查细纲**：读取 `大纲/细纲_第{A}章.md`、`细纲_第{A+1}章.md`、`细纲_第{C}章.md`（三份全文），取首份「单元ID/位置」的单元ID（若三份跨单元，以覆盖章最多的单元为主，并注明跨单元），再按 SKILL.md「卷纲不整读（取段器）」跑 `outline_view.py --unit {单元ID} --stage write {卷纲路径}` 拿当前剧情单元、卷契约、全卷常任裁定、本卷主推线/战果、终局底牌边界与风险等级——**不整读卷纲**；**写正文一律 `--stage write`**，批次底稿（供给自查／建纲追加／批次级复检）是排纲期底稿，不进写作回路。脚本 exit 1 时按它的提示修，不改用整读绕过。如果不存在或缺少当前章节蓝图的必需字段，**必须先补建细纲再写正文**，不允许跳过细纲直接写作。补建时参考卷纲中本章对应的事件规划和上下文，补齐阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定；无法从已有证据判断的字段写 `[待补充]`，不杜撰副线或关系。补建后按 `artifact-protocols.md` 跑 `check-outline-contract.js` 结构验收，失败只补点名字段再复验。
2. **读取上下文**（按需选择；缺失时遵循各项及 `project-files.md` 的「缺失文件处理」，仅明确标为可选的非主产物跳过。可选快捷路径：对应 agent 已部署、**且降档不成立**时 spawn story-explorer 一次获取上下文。Prompt：`项目目录：{dir}\n查询类型：context_load\n查询参数：准备写正文第 {B} 章（细纲 {A}-{C}）\n追踪状态：last_committed_chapter={check 的值}，state_revision={check 的值}`）：
   - (1) `正文/第{B-1}章_*.md` — 上一章正文（**脚本代读结尾**；主会话只在承接存疑时才自己读）
   - (2) `大纲/细纲_第{A}章.md` … `细纲_第{C}章.md` — **三份细纲全文**（含钩子）。**主会话必读**；写时合并批准情节，压成一篇紧凑正文，禁止按细纲章数拆成三章正文或用水话拉长
   - (2a) 卷纲 — **走 `outline_view.py --unit {单元ID} --stage write`，不整读**（见 SKILL.md「卷纲不整读」）：当前剧情单元、卷契约与终局储备（主推线/战果、终局底牌边界）、全卷常任裁定
   - (3) `tracking_commit.py check` + `追踪/上下文.md` + `追踪/章号映射.md` — `check` 无 ERROR 输出即通过，从它的紧凑 JSON 取 `last_committed_chapter` / `state_revision`，不把完整 state 加入 prompt。**续写状态卡整份不注入 prompt**：脚本只校验 7 栏完整，状态由主会话读全 7 栏后筛进步骤 7 的「本节速记」槽。汇报进度时同时报正文第几章与细纲消费到第几章
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **指令确认**：综合细纲、本节速记和模块召回结果，用一句话写清本章意图。
	     ...
	     - **细纲优先边界**：正文以**本批三份细纲（A–C）合并后的批准情节**为唯一剧情蓝图；新增物按三档授权（档表见步骤 7）——一档现场材料直接写、二档写并申报、三档（新主线事件、新反转、新金手指规则、提前写后续章剧情、改变细纲已定结果）不写，**不要用「不自造新剧情」一刀切**。后续阶段真相、底牌、关系结论和终局矛盾不得因为章尾钩子提前泄露。反过来，细纲是"要发生什么"的契约、不是正文的形状：正文可自由编排叙述顺序、合并/穿插情节点，不必一个情节点一段、也不必按五段式顺写，把每个点演成场景而不是照抄概括语（见 writing-craft.md「从细纲到正文」）。三纲并一文时优先压缩过场与重复反应，**禁止用水话回填**凑字。
...
6. **写作准备**：全章细纲先交给 writer，供它统筹编排；默认同一 session 按前后两组交付。
   ...
   - writer 先写前组临时 segment；主会话只调用一次 `storyctl.py wordcount checkpoint --file {segment} --target {参考目标} --chapter {B}`，把 `actual / remaining_user_range` 连同尚未完成的后组交回同一 writer（remaining 仅供节奏感，**禁止据此追字**）。两组完成后按原文拼接，不回改前组追字数。参考目标可取三份细纲目标之和，**绝不强制贴合**。
   - 用户明确要求一次成文时，执行安排填「全章」，直接落盘；不额外跑 checkpoint。无论哪种方式，批准内容写完即停，不为字数加剧情，**禁止用水话回填**，也**禁止为凑满 2000 或目标带而加厚**。
   - 组装后的实际长度由步骤 8 的 `chapter check` 收口并记账；`under`/`over` 相对细纲目标只记录，不追写；情节齐即接受当前长度。
   - **正文元信息隔离**：`章节：第{B}章`、`上一章：正文/第{B-1}章_*.md`、`匹配第K章`、`细纲文件：第{A}-{C}章` 等只用于定位材料。标题行以外的正文不得出现 `第[一二三四五六七八九十百千万两0-9]+章|上一章|上章|前一章|本章|这一章|前文|后文|伏笔|细纲|读者` 这类写作工程词。需要承接前文时，改成角色能感知的事件锚点或相对时间，例如“比第一章那三秒开火更疼”必须写成“比那三秒开火更疼”。例外：角色在故事世界内真实阅读/讨论“第X章”文本，或真实身为作者/读者并谈论读者身份时，可保留相应词。
7. **正文执行**：
...
8. **先核时空表，再非对称收口**：
   ...
   - **字数记账（必做）**：用 check 返回的 `length.actual`（或 `wordcount measure`）写入独立文件 `追踪/字数记录/第{NNN}章.md`（见下方「字数测量权威」模板）。此文件只记账，不触发改写。
   - 带内 + pass → 完成步骤 9-12 的语义核对与新增物裁定后再 `chapter commit`；fail / `invalid` → 停止。
   - `under` + pass → **禁止**加厚/水话/合并催长。情节齐 → 直接 `accept-current-length`（不论是否 <2000）；情节有漏 → 只补批准内容后复检，仍短则照样接受。
   - `over` + pass → 可选一次净删（`compress-once`）去注水；零新语义。不追目标带；也可直接 `accept-current-length`。
...
12. **更新追踪**：最终 `chapter check` 后取最新 `state_revision`，按 workflow-daily 构造不含 `wordcount` 的逐章 JSON（`chapter` = 正文章号 B）。`delta.result` 须含映射句：`正文第{B}章 ← 细纲第{A}–{C}章`。确认 `追踪/字数记录/第{NNN}章.md` 已写入后执行 `storyctl.py chapter commit` 或 `accept-current-length`（相对细纲目标 `under`/`over` 时用后者）。提交成功后**立即更新** `追踪/章号映射.md`（正文已写到 B、细纲已消费到 C）。写入失败保留原事务重跑；校验失败重构事务。派生视图损坏按 tracking 文档用 revision 修复，禁止手改。本章首次引入复用角色/势力时，仍按 workflow-setup Phase 3 补建静态档案。**判为「收编」的申报项在这一次事务里一并登记**（映射见「Agent 调用：consistency-checker」的三档表）：漏登记等于没写。有「提请作者确认」项时本章暂停提交、不得继续下一章，裁定后按下表处理；判为「修复」的按修复后的结果登记。
...
字数口径：`visible_chars_v1`。**不设硬合格线、不追细纲目标字数**：生成多少就是多少；比 2000 少也不加厚、不合并、不水话回填。细纲 `字数目标` 与工具 ±12%/±15% 带仅作规划/测量参考。人味/去AI/压缩后变短（少了就少了）照样提交。相对目标 `under`/`over` → 默认 `accept-current-length`（`over` 仅在明显注水时可选一次净删）。本轮用户明确给出上下限时，原样范围优先。下一章只看 tracking 提交 + `追踪/章号映射.md`。

**独立字数账本（必写）**：每章定稿（提交前）写入 `追踪/字数记录/第{NNN}章.md`，与 `_tracking-state.json` 并行，不混入正文、不据此改写：

```markdown
# 第{N}章字数记录
- 正文文件：正文/第{NNN}章_{章名}.md
- 口径：visible_chars_v1
- 实际字数：{actual}
- 细纲参考目标：{target}（仅存档，不追写）
- 相对目标状态：{internal_pass|borderline|under|over}
- 处置：accept-as-is
```

写完正文提交追踪后，**必须**更新 `追踪/章号映射.md`：正文已写到第 B 章、细纲已消费到第 C=3B 章、最近映射句。
