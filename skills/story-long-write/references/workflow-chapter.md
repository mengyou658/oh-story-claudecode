...
6. **写作准备**：全章细纲先交给 writer，供它统筹编排；默认同一 session 按前后两组交付。
   ...
   - writer 先写前组临时 segment；主会话只调用一次 `storyctl.py wordcount checkpoint --file {segment} --target {参考目标} --chapter {B}`，把 `actual / remaining_user_range` 连同尚未完成的后组交回同一 writer。两组完成后按原文拼接，不回改前组追字数。参考目标可取三份细纲目标之和，**仍不强制贴合**；合格线见字数测量权威（≥2000）。
   - 用户明确要求一次成文时，执行安排填「全章」，直接落盘；不额外跑 checkpoint。无论哪种方式，批准内容写完即停，不为字数加剧情，**禁止用水话回填**。
   - 组装后的实际长度由步骤 8 的 `chapter check` 收口；`under` 且 ≥2000 直接接受自然长度；仅 <2000 才处置；超长最多净删一次。
...
8. **先核时空表，再非对称收口**：
   ...
   - `under` + pass → **禁止**加厚/水话回填。实际 ≥2000 → 直接 `accept-current-length`；实际 <2000 → 先补未落地批准情节，情节已齐则合并相邻正文或请用户接受更短长度。
...
字数口径：`visible_chars_v1`。**硬合格线：每章正文 ≥2000 字**（本仓库覆盖；见 [local-production-rules.md](local-production-rules.md) §3）。细纲 `字数目标` 与工具 ±12%/±15% 带仅作规划/测量参考，**不强制符合目标字数**；人味/去AI/压缩后变短（少了就少了）只要仍 ≥2000 即可提交，**禁止用水话回填**，优先**紧凑快节奏**。`under` 且 ≥2000 → `accept-current-length`；`under` 且 <2000 → 禁止加厚/水话，默认合并相邻两章正文并回写章号映射。`over` 最多净删一次。本轮用户明确给出上下限时，原样范围优先。下一章只看 tracking 提交 + `追踪/章号映射.md`。
