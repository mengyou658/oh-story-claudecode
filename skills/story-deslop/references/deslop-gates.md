...
**真删，不换汤**（shuorenhua / hanyu）：「首先→首先呢」、黑话换黑话、空洞副词换同义空洞词，一律算失败。能删就删；删不掉才进 Gate 改表达。保真、scope、情态与无源论断场景分流见 [fidelity-constraints.md](fidelity-constraints.md)。

1. 删掉后是否丢失伏笔、钩子、角色特征、情节推进、必要信息或必要转折？都不丢则直接删，不进 Gate。
2. 丢任意一项 → 保留信息进对应 Gate 改写（只删"怎么说"的 AI 味，不删"说什么"；条件/否定/情态/归属不漂移）。
...
- 删模糊归因（slop-gauge `vague_attribution`）：「研究表明 / 专家表示 / 观察者指出」无出处 → **novel 叙述层**删或压成角色可见信息（不编造出处）；**article/短文**默认保留归属+论断，见 fidelity-constraints 场景分流。
