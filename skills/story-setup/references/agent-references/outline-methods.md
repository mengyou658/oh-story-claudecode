...
**默认取法（断章取义）**：从本章正文里挑一句**重要转场句**（场面切换、态度翻转、信息落地、行动开启处），再**截取约一半**作最终章名——像把原句拦腰掐断，留下半截钩子，不求语义完整。例：正文有「剑宗的穷，震耳欲聋」→ 章名可取「剑宗的穷」或「穷得震耳欲聋」；有「我也该出发了」→ 可取「我也该出发」。细纲标题、正文标题行、文件名三者必须一致。

1. 禁止用干瘪概括（"开始""觉醒""升级"）
2. 优先转场句半截；其次才用最有梗的整句或最冲突的信息
3. 铺垫章标题也要有噱头，同样走断章取义
4. 避免"分析""局势""博弈"等字眼
5. 同名或明显重复时换另一句转场句再截半截，保持细纲与文件名同步
