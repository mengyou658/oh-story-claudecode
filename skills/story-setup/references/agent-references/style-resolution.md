...
**基调默认：轻快 + 欢乐 + 沉稳。** 短句推进、笑点落地即走、情绪不滥不闹；不堆冷硬虐感，也不狂躁沙雕盖过剧情。

**默认文风对标书**：`拆文库/我的师兄们，帅是真帅，穷是真穷/`（其 `文风.md` 与原文锚点）。开书/写正文时若本书无可用 `设定/文风.md`、且未登记其他主对标文风，按此书补齐表达维度；本书已写明文风或当前请求另有指定时，本默认只补未声明维度。

## 按维度取值，不把整份文件互相覆盖

表达选择按以下顺序取第一个明确、适用的要求：**当前请求 > 本书文风/设定 > 本书 active 作者记忆 > 题材、流程、全局 active 记忆 > 主对标文风 > 全局默认文风底（轻快+欢乐+沉稳 / 《我的师兄们》）> 题材包与通用 references 默认值**。未声明的维度才向下补齐；本书例外不撤销全局习惯，临时要求不写回长期记忆。
