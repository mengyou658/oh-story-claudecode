...
| `story-setup/references/agent-references/phrase-bank-humanize.md` | 对照库替换（精确 map + **逐句语义对齐**）+ ainovel 判据；**硬步骤**（报告须 `对照库:已执行`）；**先**备份到 `_revision-backups/`，再写出 `{stem}_humanized{ext}` |
